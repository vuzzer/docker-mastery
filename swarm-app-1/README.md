...
docker service create --name redis --network backend --network frontend redis:3.2
...
docker service create --name worker --network backend bretfisher/examplevotingapp_worker
...
docker service create --name db --network backend -e POSTGRES_HOST_AUTH_METHOD=trust --mount source=db-data,target=/var/lib/postgresql/data postgres:9.4
